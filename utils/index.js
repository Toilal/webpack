...
/**
 * Runs `npm run tslint -- --fix` in the project directory
 * @param {string} cwd Path of the created project directory
 * @param {object} data Data from questionnaire
 */
exports.runTslintFix = function runTslintFix(cwd, data, color) {
  if (data.lint && lintStyles.indexOf(data.tslintConfig) !== -1) {
    console.log(
      `\n\n${color(
        'Running tslint --fix to comply with chosen preset rules...'
      )}`
    )
    console.log('# ========================\n')
    const args =
